    ...
    <div className="relative min-h-full">
      ...
      {searchOpen ? (
        ...
        >
          <div
            className={cn(
              "overflow-hidden rounded-2xl border border-white/[0.14] bg-[#1c1c1e]/95 text-foreground shadow-2xl",
          ...
          >
            <div
              className={cn(
                "flex items-center gap-2.5 border-b border-white/10 px-3.5 py-2.5 sm:px-4 sm:py-3",
              )}
            >
              <Search
              ...
              <input
                ...
                className={cn(
                  ...
                  /* 16px base: iOS avoids auto-zoom on focused inputs smaller than 16px */
            ...
            <ul
              className="max-h-[min(52dvh,420px)] overflow-y-auto overscroll-contain px-2 py-2 sm:px-3"
              aria-label="Apps matching search"
            >
              {filtered.length === 0 ? (
                <li className="px-2 py-6 text-center text-sm text-muted-foreground">
                  {query.trim().length > 0
                    ? `No apps match “${query.trim()}”.`
                    : "No apps in your library yet."}
                </li>
              ) : (
                filtered.map((app) => (
                  <li key={app.slug}>
