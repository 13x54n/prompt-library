...
function FooterContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5",
        className,
      )}
      {...props}
    />
...
function FooterBottom({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "border-border dark:border-border/15 text-muted-foreground mt-8 flex w-full min-w-0 flex-col items-center justify-between gap-4 border-t pt-4 text-xs sm:flex-row sm:items-center",
        className,
      )}
      {...props}
    />
