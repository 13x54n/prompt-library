...
async function getChannel() {
  ...
  try {
    ...
    console.error("[auth-service] RabbitMQ unavailable. Will retry on next publish:", err.message);
    return null;
  }
}

function buildDomainEvent(eventType, payload) {
  return {
    eventId: randomUUID(),
    eventType,
...
export async function publishDomainEvent(eventType, payload) {
  ...
  try {
    ...
    console.error("[auth-service] Failed to publish event:", err.message);
    channel = null;
    connection = null;
    return false;
  }
